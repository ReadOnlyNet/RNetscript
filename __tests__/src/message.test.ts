...
describe('message tests', () => {
	const client: any = new Eris.Client(`Bot ${process.env.RNET_TOKEN}`, { restMode: true });
	...
	test('should send a message to discord', async () => {
		...
		await RNetscript.execute(command, {client, env});
	...
	test('should send an embed with all possible values', async () => {
		const command = `
			{embed
				...
				author: {
					name: 'Author',
					icon_url: 'https://www.rnet.cf/images/rnet-v2-300.jpg'
				}
				color: '#ffffff'
				title: 'Title'
				...
				thumbnail: 'https://www.rnet.cf/images/rnet-v2-300.jpg'
				image: 'https://www.rnet.cf/images/rnet-v2-300.jpg'
				footer: {
					text: 'Footer',
					icon_url: 'https://www.rnet.cf/images/rnet-v2-300.jpg'
				}
